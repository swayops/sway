...
func getDealsForInfluencer(s *Server) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		deals := inf.GetAvailableDeals(s.Campaigns, s.Audiences, s.budgetDb, "", "",
...
func getDeal(s *Server) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		deals := inf.GetAvailableDeals(s.Campaigns, s.Audiences, s.budgetDb, campaignId, dealId, nil, true, s.Cfg)
		if len(deals) != 1 {
...
func assignDeal(s *Server) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		currentDeals := inf.GetAvailableDeals(s.Campaigns, s.Audiences, s.budgetDb, campaignId, dealId, nil, false, s.Cfg)
