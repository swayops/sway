...
func GenerateCampaignReport(res http.ResponseWriter, db, statsDb *bolt.DB, cid string, from, to time.Time, cfg *config.Config) error {
	...
	st, err := getTargetStats(cid, statsDb, cfg, from, to)
	if err != nil {
		return err
	}
...
func setChannelLevelSheet(xf Sheeter, from, to time.Time, channel map[string]*ReportStats) {
	...
	var totalEng float32
	for _, st := range channel {
		totalEng += float32(st.Likes + st.Comments + st.Shares)
	}

	for platform, st := range channel {
		eng := (float32(st.Likes+st.Comments+st.Shares) / totalEng) * 100
		sheet.AddRow(
			platform,
			st.Likes,
			st.Comments,
...
func getPerc(val float32) string {
	if val < 1 {
		return "<1%"
	}
